...
namespace ZeroTrustMigrationAddin.Services
{
    ...
    public class MockDataService
    {
        ...
        public async Task<List<Workload>> GetWorkloadsAsync()
        {
            ...
            // Co-managed devices = 55,900. Workload authority counts per workload.
            // Completed workloads: ~95% of co-managed devices have Intune authority
            // InProgress: partial rollout
            // NotStarted: 0% Intune authority — all devices still on ConfigMgr for this workload
            return new List<Workload>
            {
                new Workload { Name = "Compliance Policies", Status = WorkloadStatus.Completed, Description = "Device compliance policies moved to Intune", TransitionDate = DateTime.Now.AddMonths(-3), Order = 1, IntuneAdoptionPercentage = 95, IntuneDeviceCount = 53105, ConfigMgrDeviceCount = 2795, HasRealData = false, ReadinessScore = 100, RiskLevel = "Low", EstimatedTime = "Complete", Benefits = new() { "Continuous compliance evaluation via Intune", "Conditional Access enforcement" } },
                new Workload { Name = "Device Configuration", Status = WorkloadStatus.Completed, Description = "Configuration profiles migrated", TransitionDate = DateTime.Now.AddMonths(-2), Order = 2, IntuneAdoptionPercentage = 92, IntuneDeviceCount = 51428, ConfigMgrDeviceCount = 4472, HasRealData = false, ReadinessScore = 100, RiskLevel = "Low", EstimatedTime = "Complete", Benefits = new() { "Cloud-based security baselines", "Settings Catalog flexibility" } },
                new Workload { Name = "Endpoint Protection", Status = WorkloadStatus.Completed, Description = "Antivirus and security settings", TransitionDate = DateTime.Now.AddMonths(-1), Order = 3, IntuneAdoptionPercentage = 91, IntuneDeviceCount = 50869, ConfigMgrDeviceCount = 5031, HasRealData = false, ReadinessScore = 100, RiskLevel = "Low", EstimatedTime = "Complete", Benefits = new() { "Defender for Endpoint cloud integration", "Attack Surface Reduction rules" } },
                new Workload { Name = "Windows Update for Business", Status = WorkloadStatus.InProgress, Description = "Windows update policies", LearnMoreUrl = "https://docs.microsoft.com/mem/intune/protect/windows-update-for-business-configure", Order = 4, IntuneAdoptionPercentage = 62, IntuneDeviceCount = 34658, ConfigMgrDeviceCount = 21242, HasRealData = false, ReadinessScore = 72, RiskLevel = "Medium", EstimatedTime = "4-6 weeks", Benefits = new() { "Eliminate WSUS infrastructure", "Cloud-intelligent update delivery" } },
                new Workload { Name = "Client Apps", Status = WorkloadStatus.NotStarted, Description = "Application deployment and management", LearnMoreUrl = "https://docs.microsoft.com/mem/intune/apps/apps-add", Order = 5, IntuneAdoptionPercentage = 0, IntuneDeviceCount = 0, ConfigMgrDeviceCount = 55900, HasRealData = false, ReadinessScore = 35, RiskLevel = "High", EstimatedTime = "6-8 weeks", Benefits = new() { "Win32 app management from cloud", "Reduce distribution point dependency" }, DependsOn = new() { "Compliance Policies" } },
                new Workload { Name = "Office Click-to-Run Apps", Status = WorkloadStatus.NotStarted, Description = "Microsoft 365 Apps management", LearnMoreUrl = "https://docs.microsoft.com/mem/intune/apps/apps-add-office365", Order = 6, IntuneAdoptionPercentage = 0, IntuneDeviceCount = 0, ConfigMgrDeviceCount = 55900, HasRealData = false, ReadinessScore = 40, RiskLevel = "Medium", EstimatedTime = "3-4 weeks", Benefits = new() { "Cloud-managed M365 Apps updates", "Servicing profiles" }, DependsOn = new() { "Client Apps" } },
                new Workload { Name = "Resource Access Policies", Status = WorkloadStatus.NotStarted, Description = "VPN, Wi-Fi, and certificate profiles", LearnMoreUrl = "https://docs.microsoft.com/mem/intune/configuration/device-profiles", Order = 7, IntuneAdoptionPercentage = 0, IntuneDeviceCount = 0, ConfigMgrDeviceCount = 55900, HasRealData = false, ReadinessScore = 25, RiskLevel = "High", EstimatedTime = "4-6 weeks", Benefits = new() { "Cloud-based certificate deployment", "Always-on VPN via Intune" } }
            };
        }
